...
async def trigger_training(body: TrainRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        logger.exception(
            "[%s] Could not persist initial training run state",
            run_id,
            exc_info=exc,
        )
...
async def get_training_status(run_id: str):
    ...
    db_run = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: load_training_run(run_id),
    )
    if db_run is None:
...
async def _run_training(run_id: str, params: TrainRequest, started_at: datetime) -> None:
    ...
    try:
        ...
        ohlcv_map = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: load_ohlcv(params.symbols, min_rows=WINDOW + HORIZON + 10),
        )
        if not ohlcv_map:
            raise RuntimeError("No symbols with enough data found. Run the market data seed first.")
        ...
        def _norm_seqs(sequence_batch: np.ndarray) -> np.ndarray:
            n_s, n_f, w = sequence_batch.shape
            flat = sequence_batch.transpose(0, 2, 1).reshape(-1, n_f)
        ...
        logger.info(
            ...
            train_result.stopped_early,
        )

        evaluator = Evaluator(model)
        eval_metrics = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: evaluator.evaluate(loaders.test),
        )
        ...
        logger.info(
            "[%s] Completed - version_id=%s  test_acc=%.4f",
            run_id,
            version_id,
