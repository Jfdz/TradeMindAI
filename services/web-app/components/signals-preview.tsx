  ...
  return (
    <section className="border-t border-slate-200 py-8 dark:border-white/10">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {signals.length > 0 ? (
          signals.map((signal) => (
            <article key={signal.id} className="rounded-3xl border border-slate-200 bg-slate-100 p-5 dark:border-white/10 dark:bg-white/5">
              <p className="text-xs uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400">{signal.timeframe}</p>
              <h4 className="mt-3 text-xl font-semibold text-slate-900 dark:text-white">{signal.type}</h4>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Confidence {(signal.confidence * 100).toFixed(1)}%</p>
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Generated at {new Date(signal.generatedAt).toLocaleString()}</p>
            </article>
          ))
        ) : (
          <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-600 md:col-span-3 dark:border-white/15 dark:bg-white/5 dark:text-slate-300">
            {error
