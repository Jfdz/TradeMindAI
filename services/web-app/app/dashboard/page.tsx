const summaryCards = [
  { label: "Total signals", value: "128", detail: "+18 this week", tone: "text-gold-300" },
  { label: "Portfolio value", value: "$82.4k", detail: "+4.8% today", tone: "text-white" },
  { label: "Top performer", value: "AAPL", detail: "+12.4% unrealized", tone: "text-mint-300" },
  { label: "Win rate", value: "61%", detail: "Last 30 days", tone: "text-gold-300" },
];
    ...
    <div className="space-y-8">
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {summaryCards.map((card) => (
          <article key={card.label} className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-glow">
      ...
      <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-glow">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-gold-300/80">Recent signals</p>
              <h3 className="mt-3 text-2xl font-semibold text-white">Last five signals in the workspace</h3>
            </div>
          ...
          <div className="mt-6 space-y-4">
            {recentSignals.map((signal) => (
              <div
                ...
                className="grid gap-3 rounded-2xl border border-white/10 bg-ink-800/70 p-4 sm:grid-cols-[1fr_auto_auto_auto]"
              >
                <div>
                ...
                <div className="text-sm text-slate-200">{signal.type}</div>
                <div className="text-sm text-slate-200">{signal.confidence} confidence</div>
                <div className="text-sm text-slate-200">{signal.price}</div>
              </div>
            ))}
          </div>
        </article>
        ...
        <article className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-glow">
          <p className="text-xs uppercase tracking-[0.35em] text-gold-300/80">Workspace notes</p>
          <h3 className="mt-3 text-2xl font-semibold text-white">Overview details</h3>

          <div className="mt-6 space-y-4">
            {activityNotes.map((item) => (
              <div key={item} className="rounded-2xl border border-white/10 bg-ink-800/70 p-4 text-sm text-slate-200">
                {item}
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 rounded-3xl border border-gold-300/20 bg-gradient-to-br from-gold-300/10 to-mint-400/10 p-5">
            <p className="text-xs uppercase tracking-[0.35em] text-gold-300/80">Next actions</p>
